...
<section class="panel">
  ...
  <div class="panel-body">
    ...
    <div>Theme</div>
     <select id="classSelector" onChange="$('#myGrid').attr('class', 'my-grid ' + this.value); $('#myChart').attr('class', 'my-chart ' + this.value);">
    ...
    <!-- CHANGE FOR ADVANCED FILTER - 4 -->
    <input type="checkbox" name="test" id="test" value="1" data-plugin-ios-switch="" style="display: none;">

    <!-- CHANGE FOR ADVANCED FILTER - 5 -->
    <script>
      const checkbox = document.getElementById('test');
      document.addEventListener('DOMContentLoaded', function() {
          
          // Function to set grid options based on checkbox state
...
<script>
  ...
  const gridOptions = {
      ...
      if (tempName) {
        getDataFromAPI(1, templateId, tempName, templateDescription, typeId, (error, data) => {
          if (error) {
            ...
            gridApi.setGridOption("", toppivot);

            console.log("Fetched Filter Model",data.filter.advancedFilterModel);
    ...
    onStateUpdated: event => {
      ...
      let state=gridApi.getState();
      console.log(state.filter.advancedFilterModel);
